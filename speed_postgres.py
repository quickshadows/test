...
# ------------------ Параметры ------------------
PGSQL_HOST = input("IP или хост PostgreSQL: ").strip()
PGSQL_PORT = 5432
PGSQL_USER = "gen_user"
...
txt_file = Path(f"sysbench_pg_{PGSQL_HOST}_{timestamp}.txt")
csv_file = Path(f"sysbench_pg_{PGSQL_HOST}_{timestamp}.csv")
...
stdout, stderr, rc = run_sysbench([
    "--db-driver=pgsql",
    SCRIPT,
    f"--pgsql-host={PGSQL_HOST}",
    f"--pgsql-port={PGSQL_PORT}",
    f"--pgsql-user={PGSQL_USER}",
    f"--pgsql-password={PGSQL_PASSWORD}",
    f"--pgsql-db={PGSQL_DB}",
    f"--tables={TABLES}",
...
print("✅ Тест PostgreSQL завершён!")
